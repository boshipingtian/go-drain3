...
//   - bool: 删除是否成功
//   - error: 删除过程中的错误信息
func (m *TemplateMiner) DeleteCluster(ctx context.Context, clusterId int64) (bool, error) {
	deleted, err := m.drain.DeleteCluster(clusterId)
	if err != nil {
	...
	if deleted {
		// 删除成功后保存状态
		if err := m.SaveState(ctx); err != nil {
			return false, fmt.Errorf("failed to save state after deletion: %w", err)
		}
	}
